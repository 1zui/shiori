      ...
      <div className="max-w-[1600px] mx-auto flex flex-col gap-8">
        ...
        <div className="w-full bg-[#1a1a1a] border border-slate-850 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row gap-6 md:gap-8 items-start relative overflow-hidden shadow-2xl group">
          ...
          <div className="flex-1 flex flex-col justify-between h-full space-y-4 z-10 relative">
            <div>
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight text-white mb-3">
                {manga.title}
              </h1>

              <div className="flex flex-wrap gap-2 mb-4">
                <span className="text-[10px] font-black tracking-widest uppercase bg-orange-600/10 border border-orange-500/20 text-orange-400 px-3 py-1 rounded-md">
                  {manga.status || "Ongoing"}
                </span>
                <span className="text-[10px] font-black tracking-widest uppercase bg-slate-800 text-slate-300 px-3 py-1 rounded-md border border-slate-700">
                  {manga.year || "N/A"}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {sortedChapters.length === 0 ? (
              <div className="text-center py-12 bg-[#1a1a1a] rounded-2xl text-xs text-slate-500 border border-slate-850">
              ...
              <div className="flex flex-col gap-2.5 max-h-[650px] overflow-y-auto pr-2 scrollbar-thin">
                {sortedChapters.map((chap: MangaChapter) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-3 bg-[#1a1a1a] hover:bg-[#222] border border-slate-850/50 hover:border-slate-700 rounded-2xl group transition-all shadow-sm"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-extrabold px-1.5 py-0.5 bg-[#121212] border border-slate-800 rounded text-orange-400 tracking-wider">
                            {chap.language}
                          </span>
                        </div>
                        {chap.title && (
                          <span className="text-xxs text-slate-400 font-medium line-clamp-1">
                            {chap.title}
                          </span>
                        )}
                      </div>
