import { getPopularManhwa, getPopularManga } from "@/lib/mangadex";
...
export default async function ManhwaPage({ searchParams }: PageProps) {
  const resolvedParams = await searchParams;
  const query = resolvedParams.q || "";
  const page = Number(resolvedParams.page) || 1;

  // 💡 DI-FIX: Panggil fungsi Manhwa Korea & bongkar object datanya secara dinamis
  const { items, totalPages } = await getPopularManhwa(page);
  
  // Ambil data untuk sidebar trending 10 besar global
  ...
  return (
    <main className="max-w-[1800px] mx-auto p-6 md:p-12 text-slate-900 dark:text-white transition-colors">
      ...
      {/* ─── KODE HEADER BERSIH KHUSUS Halaman Manhwa ─── */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 border-b border-slate-200 dark:border-slate-900 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Discover highly popular and top-voted Korean Webtoons / Manhwa collections
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          {items.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                {items.map((manga) => (
                  <Link href={`/manga/${manga.id}`} key={manga.id} className="group block">
                    ...
                    <h3 className="text-xs font-bold mt-2 text-slate-800 dark:text-slate-200 line-clamp-1 group-hover:text-orange-500 transition-colors">{manga.title}</h3>
                  </Link>
                ))}
              </div>

              {/* 💡 DI-FIX: Properti baseUrl diganti ke "/manhwa" dan totalPages dinamis */}
              <Pagination 
                currentPage={page} 
        ...
        <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-900 pt-8 lg:pt-0 lg:pl-8">
          <h3 className="text-sm font-black tracking-widest uppercase text-slate-400 mb-6">🔥 Trending Now</h3>
          <div className="flex flex-col gap-4">
            {top10Trending.map((manga, idx) => (
              <Link href={`/manga/${manga.id}`} key={manga.id} className="flex items-center gap-3 group">
                <div className="text-sm font-black text-slate-400 w-5 text-center group-hover:text-orange-500">{idx + 1}</div>
                ...
                <div className="text-xs font-bold line-clamp-2 text-slate-700 dark:text-slate-300 group-hover:text-orange-500 transition-colors">{manga.title}</div>
              </Link>
            ))}
          </div>
        </div>
