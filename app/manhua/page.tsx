import { getPopularManhua, getPopularManga } from "@/lib/mangadex";
...
export default async function ManhuaPage({ searchParams }: PageProps) {
  const resolvedParams = await searchParams;
  const query = resolvedParams.q || "";
  const page = Number(resolvedParams.page) || 1;

  // 💡 DI-FIX: Panggil fungsi Manhua Cina, bukan Manga Jepang lagi
  const { items, totalPages } = await getPopularManhua(page);
  ...
  return (
    <main className="max-w-[1800px] mx-auto p-6 md:p-12 text-slate-900 dark:text-white transition-colors">
      ...
      {/* ─── KODE HEADER BERSIH KHUSUS Halaman Manhua ─── */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 border-b border-slate-200 dark:border-slate-900 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Discover highly popular and trending Chinese Manhua collections
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3">
          {items.length === 0 ? (
            ...
            <>
              ...
              {/* 💡 DI-FIX: totalPages diikat dinamis ke variabel {totalPages} bawaan API */}
              <Pagination 
                currentPage={page} 
